...
const TeamProfileView = memo(function TeamProfileView({
  entries,
  teamKey,
  onNavigate,
  onGoToSeason,
  ...
  const stageRostersRef = useRef<HTMLDivElement>(null);
  const goToTeamSeason = useCallback(
    (seasonId: string) => {
      const idx = seasonsWithStages.findIndex((s) => s.seasonId === seasonId);
      if (idx >= 0) {
        setSeasonIdx(idx);
        setStageIdx(0);
        requestAnimationFrame(() => {
          stageRostersRef.current?.scrollIntoView({ behavior: "smooth", block: "nearest" });
        });
        return;
      }
      onGoToSeason?.(seasonId);
    },
    [seasonsWithStages, onGoToSeason],
  );
  const [previousSelection, setPreviousSelection] = useState({ teamKey, seasonIdx });
  if (previousSelection.teamKey !== teamKey || previousSelection.seasonIdx !== seasonIdx) {
    setPreviousSelection({ teamKey, seasonIdx: previousSelection.teamKey !== teamKey ? 0 : seasonIdx });
    if (previousSelection.teamKey !== teamKey) setSeasonIdx(0);
    setStageIdx(0);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <TeamSeasonResultsHistory seasons={t.seasons} onGoToSeason={goToTeamSeason} />
                        ...
                        <div key={`${s.season}-${s.archivedAt}`} className="relative flex items-center">
                          ...
                          <div className="relative z-[1] flex flex-col items-center gap-0.5">
                            <button
                              ...
                              className={`flex flex-col items-center gap-1 px-1.5 transition-colors ${
...
const CoachProfileView = memo(function CoachProfileView({
  entries,
  name,
  onNavigate,
  onGoToSeason,
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          {c.tenures.map((t, i) => (
            <SeasonScope.Provider key={i} value={t.seasonId}>
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border border-rift-line/30 bg-rift-bg/20 px-2.5 py-1.5 text-[10px] min-w-0">
              ...
              <button type="button" onClick={() => onNavigate("teams", teamRefKey(t.team))} className="min-w-0 max-w-full overflow-hidden hover:opacity-75 transition-opacity" title={`View ${t.team.name}`}>
                <TeamRef team={t.team} size={13} nested seasonId={t.seasonId} />
              </button>
...
function SearchPanel({
  ...
  const [previousNav, setPreviousNav] = useState<typeof initialNav>(undefined);
  if (initialNav && previousNav !== initialNav) {
    setPreviousNav(initialNav);
    setKind(initialNav.kind);
    setSelected(initialNav.id);
    setSortKey("name");
    setQuery("");
  }
  useEffect(() => { if (initialNav) onNavConsumed?.(); }, [initialNav, onNavConsumed]);
...
export default function SeasonHistoryView({
  ...
  const goToSeasonInTimeline = useCallback(
    (seasonId: string) => {
      selectTab("timeline");
      selectTimelineView("seasons");
      selectSeason(seasonId);
    },
    [selectTab, selectTimelineView, selectSeason],
  );
  const [historyPreview, setHistoryPreview] = useState<{ entries: SeasonHistoryEntry[]; message: string; previous: SeasonHistoryEntry[] } | null>(null);
  ...
  const handleImportBuffers = async (buffers: ArrayBuffer[]) => {
    ...
    if (allEntries.length === 0) {
      ...
      const previous = useDraftStore.getState().seasonHistory;
      const entries = [...new Map(allEntries.map(e => [e.id, e])).values()];
      const updated = entries.filter(e => previous.some(old => old.id === e.id)).length;
      setHistoryPreview({ entries, previous,
        message: `${entries.length} seasons: ${entries.map(e => e.name).join(", ")}. ${updated} existing seasons will be replaced; ${entries.length - updated} added. ${failed ? `${failed} files rejected: ${firstError}` : "All selected files were read."}` });
    }
  ...
  return (
    ...
    <div className="min-h-screen px-4 py-10 md:py-14">
      <div className="max-w-6xl mx-auto">
        ...
        {needsHistory ? (
          ...
          <SearchPanel
            ...
            onGoToSeason={goToSeasonInTimeline}
            liveInactive={liveSearchOpts?.liveInactive}
            liveRosterIds={liveSearchOpts?.liveRosterIds}
          />
        ) : deferredTab === "compare" ? (
      ...
      <Modal open={historyPreview !== null} title="Review history import" beforeConfirm={backupBeforeDestructiveChange} message={historyPreview?.message ?? ""} confirmLabel="Import"
        onCancel={() => setHistoryPreview(null)} onConfirm={() => {
          const plan = historyPreview; setHistoryPreview(null); if (!plan) return;
          if (useDraftStore.getState().seasonHistory !== plan.previous) { setExportMsg({ where: "import", kind: "err", text: "History changed. Review the files again." }); return; }
          const result = importSeasonHistory(plan.entries);
          setExportMsg({ where: "import", kind: "ok", text: `Imported ${result.added} seasons; replaced ${result.updated}.` });
        }} />
      <Modal
